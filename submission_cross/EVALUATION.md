...
```console
submission_cross/scripts/build_task.sh submission_cross
```
...
`model/he_mlp_weights.pth` is committed. To retrain from scratch (~40 min on
CPU) delete it and rerun, or:

```console
python3 submission_cross/model/train_he_mlp.py --epochs 30
python3 submission_cross/model/export_weights.py   # folds BatchNorm, re-checks
```
...
submission_cross/scripts/run_cross_benchmark.sh 0 --seed 3 --num_runs 3

# small instance (15 inferences, with the accuracy comparison)
submission_cross/scripts/run_cross_benchmark.sh 1 --seed 3 --num_runs 2
```

The wrapper sets `FHE_SUBMISSION_DIR=submission_cross` and calls the harness's
own `run_submission.py`; to drive it directly:

```console
FHE_SUBMISSION_DIR=submission_cross python3 harness/run_submission.py 1 --seed 3 --num_runs 2
```
...
```console
submission_cross/build/server_stop 1
```
...
```console
submission_cross/scripts/run_tpu_profile.sh
```
...
## 6. The OpenFHE CPU baseline, for comparison  *(~15 min)*

The reference submission is untouched and runs from the same harness with
`FHE_SUBMISSION_DIR` unset:
...
Expect ~8 s per encrypted inference (stage 7: 10.2 s single, 119.3 s for the
15-sample small instance) and the same 14/15 accuracy.
...
`python3 submission_cross/profile/report.py --sizes 0,1` prints a consolidated
...
full log is at `submission_cross/run/<instance>/cross_server.log`.
