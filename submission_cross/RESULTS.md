# CROSS on TPU v6e — FHE-Benchmarks-ML-Inference (MNIST) results
...
```console
python3 submission_cross/profile/trace_breakdown.py \
    submission_cross/results/tpu_profile/traces/8chip_lb1 --iterations 3
```

The saved output, including CROSS's name-keyword view for comparison, is in
`results/tpu_profile/op_breakdown_8chip.txt`.

## 4. End-to-end benchmark stages

Both submissions driven by the same unmodified `harness/run_submission.py`, on
the same idle host, `--seed 3`.

### Single instance (1 inference)

| Stage | CROSS / TPU v6e ×8 | OpenFHE / CPU | ratio |
|---|---:|---:|---:|
| 2 client key generation | 4.72 s | 2.80 s | 0.6× |
| 3 server model preprocessing | 475.50 s | 0.011 s | — |
| 5 client input preprocessing | 0.082 s | 0.006 s | |
| 6 client encode + encrypt | 4.02 s | 0.016 s | 0.004× |
| **7 server encrypted compute** | **0.259 s** | **10.219 s** | **39×** |
| 8 client decrypt + decode | 2.65 s | 0.021 s | 0.008× |
| 9 client postprocess | 0.082 s | 0.006 s | |
| public + evaluation keys | 8.0 MB¹ | 1.4 GB | |
| encrypted input | 3.0 MB | 358.8 KB | |
| encrypted result | 517.5 KB | 69.6 KB | |
| **result** | **PASS** (3/3 runs) | **PASS** | |

Inside stage 7, the TPU `Mapping.execute` itself is **153.4 / 153.8 / 155.2 ms**
across the three runs; the rest of the stage is ciphertext file I/O.

¹ The CROSS figure is the PKE public key only — see caveat 2, evaluation and
rotation keys are regenerated server-side rather than shipped. The comparable
key volume from `estimate_live_memory()` is 12.0 MiB evaluation key + 1.57 GiB
of rotation keys.

### Small instance (15 inferences)

| Stage | CROSS / TPU v6e ×8 | OpenFHE / CPU | ratio |
|---|---:|---:|---:|
| 3 server model preprocessing | 484.57 s | 0.011 s | — |
| 6 client encode + encrypt (15) | 7.72 s | 0.051 s | |
| **7 server encrypted compute (15)** | **0.556 s** | **119.32 s** | **215×** |
| — TPU execute only | 0.308 s → **20.5 ms / inference** | 7.95 s / inference | **388×** |
| — throughput | **48.7 inferences/s** | 0.126 inferences/s | |
| 8 client decrypt + decode (15) | 3.94 s | 0.058 s | |
| encrypted input | 45.0 MB | 5.2 MB | |
| encrypted result | 7.5 MB | 988.6 KB | |
| **encrypted model accuracy** | **14/15 = 0.9333** | 14/15 = 0.9333 | |
| harness plaintext model accuracy | 14/15 = 0.9333 | 14/15 = 0.9333 | |

Batch-1 latency does not improve with more chips — a single query occupies one
lane and the other seven are padding. Throughput does: 15 inferences cost two
8-wide `execute` calls, so per-inference time drops from 153 ms to 20.5 ms.

**The client is the weak side of this deployment.** CROSS encrypts at 1.81 s
and decrypts at 0.64 s per sample on the CPU, against OpenFHE's ~16 ms and
~21 ms. Part of that is the 16× larger ring, but the rest is that CROSS's
codec is JAX running on CPU and is not an optimized client path. For the
single instance this dominates everything the accelerator saved.
...
| Harness small-instance quality | encrypted 14/15 = 0.9333, **identical to the harness plaintext model** |
...
The HE-friendly model reaches 97.82% validation accuracy; the harness's own
ReLU reference model, trained by the harness during this run, reaches 97.62%.
